...
*Setup instructions for a gitian build of Pprfnk using a Debian VM or physical system.*
...
- [Building Pprfnk](#building-pprfnk)
...
Clone the git repositories for pprfnk and gitian and then checkout the pprfnk version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the pprfnk repository
...
To build Pprfnk (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the pprfnk repository.
...
```bash
    ...
    From https://github.com/pprfnk/pprfnk
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/pprfnk-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/pprfnk-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/pprfnk-build.assert
```
